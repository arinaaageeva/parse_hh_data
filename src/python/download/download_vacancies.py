...
from download_utils import download_vacancy_ids, download_vacancy

if __name__ == "__main__":

    parser = argparse.ArgumentParser()

    parser.add_argument("path_vacancies")
    ...
    parser.add_argument("--num_pages", type=int, default=20)
    ...
    try:
        with open(os.path.join(args.path_vacancies, "queue.json")) as fl:
            queue = json.load(fl)
        queue = queue["ids"]
        ...
        queue = download_vacancy_ids(args.area_id, specialization_ids, args.search_period, args.num_pages,
                                     args.requests_interval, args.max_requests_number)

        with open(os.path.join(args.path_vacancies, "queue.json"), "w") as fl:
            json.dump({"ids": queue}, fl)

    download_hashes = [files for _, _, files in os.walk(args.path_vacancies)]
    download_hashes = [file[:-5] for file in chain(*download_hashes)]

    queue = set(queue) - set(download_hashes)
    for vacancy_id in tqdm(queue, file=sys.stdout):
        try:
            vacancy = download_vacancy(vacancy_id, args.requests_interval, args.max_requests_number)
        except HTTPError as http_error:
            print(f"HTTP error occurred: {http_error}", file=sys.stderr)
        else:
            with open(os.path.join(args.path_vacancies, args.directory_batch, f"{vacancy_id}.json"), "w") as fl:
                json.dump(vacancy, fl)

    os.remove(os.path.join(args.path_vacancies, "queue.json"))
